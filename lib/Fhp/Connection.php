...
class Connection
{
    ...
    public function send($message)
    {
        ...
        if (false === $response) {
            throw new CurlException('Failed connection to '.$this->host.': '.curl_error($this->curlHandle), null, curl_errno($this->curlHandle), curl_getinfo($this->curlHandle));
        }

        $statusCode = curl_getinfo($this->curlHandle, CURLINFO_HTTP_CODE);
        if ($statusCode < 200 || $statusCode > 299) {
            throw new CurlException('Bad response with status code '.$statusCode, $response, $statusCode, curl_getinfo($this->curlHandle));
        }
